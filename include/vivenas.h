...
int64_t vn_lookup_inode_no(struct ViveFsContext* ctx, int64_t parent_inode_no, const char* file_name, /*out*/struct  ViveInode* inode);
int64_t vn_create_file(struct ViveFsContext* ctx, int64_t parent_inode_no, const char* file_name, int16_t mode, /*out*/ struct ViveInode* inode_out);
struct ViveFile* vn_open_file(struct ViveFsContext* ctx, int64_t parent_inode_no, const char* file_name, int32_t flags, int16_t mode);
struct ViveFile* vn_open_file_by_inode(struct ViveFsContext* ctx, int64_t ino, int32_t flags, int16_t mode);
...
int vn_rename_file(struct ViveFsContext* ctx, int64_t old_dir_ino, const char* old_name, int64_t new_dir_ino, const char* new_name);
#ifdef __cplusplus
}
#endif
#endif // vivenas_h__