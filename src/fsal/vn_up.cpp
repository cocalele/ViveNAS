...
static struct fridgethr* vn_up_fridge;
 ...
 */
static void
vn_invalidate(struct vn_fsal_export* mfe, struct vn_fsal_obj_handle* hdl)
{
 ...
 */
static void
vn_invalidate_close(struct vn_fsal_export* mfe,
 ...
 */
static void
vn_update(struct vn_fsal_export* mfe, struct vn_fsal_obj_handle* hdl)
{
	const struct fsal_up_vector* up_ops = mfe->m_export.up_ops;
	fsal_status_t status;
	struct gsh_buffdesc fh_desc;
...
struct vn_fsal_obj_handle*
	vn_rand_obj(struct vn_fsal_export* mfe)
{
	struct vn_fsal_obj_handle* res = NULL;
	...
	glist_for_each_safe(glist, glistn, &mfe->mfe_objs) {
		if (res == NULL) {
			/* Grab first entry */
			res = glist_entry(glist, struct vn_fsal_obj_handle,
				mfo_exp_entry);
			continue;
		}
 ...
 */
static void
vn_up_run(struct fridgethr_context* ctx)
{
	struct glist_head* glist, * glistn;

	glist_for_each_safe(glist, glistn, &ViveNASM.vn_exports) {
		struct vn_fsal_export* mfe;
		struct vn_fsal_obj_handle* hdl;

		mfe = glist_entry(glist, struct vn_fsal_export, export_entry);
		...
		hdl = vn_rand_obj(mfe);
		if (hdl)
			vn_invalidate(mfe, hdl);

		/* Invalidate and close a handle */
		hdl = vn_rand_obj(mfe);
		if (hdl)
			vn_invalidate_close(mfe, hdl);
	}
}
...
vn_up_pkginit(void)
{
	...
	frp.thr_max = 1;
	frp.thr_min = 1;
	frp.thread_delay = ViveNASM.up_interval;
	frp.flavor = fridgethr_flavor_looper;

	/* spawn MEM_UP background thread */
	code = fridgethr_init(&vn_up_fridge, "MEM_UP_fridge", &frp);
	if (code != 0) {
	...
	code = fridgethr_submit(vn_up_fridge, vn_up_run, NULL);
	if (code != 0) {
		LogMajor(COMPONENT_FSAL_UP,
...
vn_up_pkgshutdown(void)
{
	...
	int rc = fridgethr_sync_command(vn_up_fridge,
		fridgethr_comm_stop,
		120);
	...
	fridgethr_destroy(vn_up_fridge);
	vn_up_fridge = NULL;
